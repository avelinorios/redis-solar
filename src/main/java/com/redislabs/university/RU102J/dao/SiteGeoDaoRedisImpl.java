...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource();
            ...
            List<Map<String, String>> sitesList = new ArrayList<>();

            for (String key : keys) {
                pipeline.hgetAll(key);
            }
            List<Object> siteResponse = pipeline.syncAndReturnAll();
               // sitesList.add(siteResponse.get());
            for (Object site : siteResponse) {
                Map<String, String> siteMap = (Map<String, String>) site;
                if (!siteMap.isEmpty()) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             ...
             for (Site site : sites) {
                 ...
                 Response<Double> score = pipeline.zscore(RedisSchema.getCapacityRankingKey(), siteId.toString());
                 scores.put(siteId, score);
             }
             pipeline.sync();
